...
const NBNSBanner = () => {
  ...
  const { bannerData, banner_status_loading, delete_banner_loading, get_banner_loading } =
    useSelector((state) => state.nbnsBanner);
  ...
  const handleStatusConfirm = () => {
    dispatch(
      updateNBNSBannerStatus(
        detail?.id,
        { status: detail?.status == 1 ? 0 : 1 },
        { bannerable_id: 1, bannerable_type: 'nbns' },
        statusOpenFunction
      )
    );
  };
  ...
  const refetch = () => {
    const data = {
      ...
      bannerable_type: 'nbns',
      bannerable_id: 1
    };
    dispatch(getNBNSBanner(data));
  };
